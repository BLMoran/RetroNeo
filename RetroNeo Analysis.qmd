...
tbl_dem <- retro_dem |>  
  select(patientn = count_pt, age_pma_wks, dem_sex, dem_wgt, dem_loc, resp_support_prior, mhx_nicu, mhx_iugr, mhx_ivh, mhx_retin, mhx_ccd, mhx_cld, mhx_apn, mhx_intu, mhx_rsp, mhx_caff,  age_cat) |>  
...
tbl_surg <- retro_surg |>  
  select(patientn = count_pt, age_pma_wks, age_pma_surg_wks, dem_wgt, sur_wgt, sur_hernia, sur_type, sur_incarc, sur_asspro, total_dur, anaes_dur, surg_dur, age_cat) |>  
    tbl_summary(by = age_cat, missing = "no",
            label = list(
              patientn = "Number of Patients (N)",
              age_pma_wks = "Post-Menstrual Age at Birth in Weeks (Median (IQR))",
...
tbl_ga <- retro_anaes |>  
  select(patientn = count_pt, anaes_type, ga_type, nmba_type, rev_agent, intraop_para, intraop_parecoxib, intraop_opioid, intraop_dexmede, airway_type,  age_cat) |>  
    tbl_summary(by = age_cat, missing = "no",
            label = list(
              patientn = "Number of Patients (N)",
...
tbl_spinal <- retro_anaes |>  
  ...
  select(patientn = count_pt, spinal_drug_conc_comb, spinal_attempts, spinal_complications, spinal_ga_conv, sedation, sed_prop, sed_dexmede, sed_midaz, sed_clonidine, sed_remi, sed_ket, sed_no2, sed_sucrose, airway_type,  age_cat) |>  
    tbl_summary(by = age_cat, missing = "no",
            label = list(
              patientn = "Number of Patients (N)",
...
tbl_caudal <- retro_anaes |>  
  ...
  mutate(
    ...
    sed_ket = factor(sed_ket, levels = c("0","1")),
    sed_remi = factor(sed_remi, levels = c("0","1"))) |> 
  select(patientn = count_pt, caudal_drug_conc_comb, caudal_add, caudal_attempts, caudal_complications, caudal_ga_conv, sedation, sed_prop, sed_dexmede, sed_midaz, sed_clonidine, sed_remi, sed_ket, sed_no2, sed_sucrose, airway_type,  age_cat) |>  
    tbl_summary(by = age_cat, missing = "no",
            label = list(
              patientn = "Number of Patients (N)",
...
#ggplot()+
 # geom_bar(aes(x = anaes_type, fill = age_cat), position = "dodge")+
  #theme_bw()+
  #theme(axis.text.x = element_text(angle = 90))+
  #labs(y = "Number", x = "Anaesthetic Type", title = "Anaesthetic Type", subtitle = "By Age Categories")+
  #scale_fill_manual(name = "Age Categories at Birth", values = c("lightblue", "darkcyan", "cornflowerblue", "deepskyblue2"))
...
#retro_anaes |> 
 # dplyr::filter(!anaes_type %in% c("Caudal Only", "GA + Spinal + Caudal", "GA + Caudal + Sedation", NA)) |> 
#ggplot()+
 # geom_bar(aes(fill = anaes_type, x = age_cat), position = "dodge")+
#  theme_bw()+
 # theme(axis.text.x = element_text(angle = 90))+
#  labs(y = "Number", x = "Age Category", title = "Anaesthetic Type")+
 # scale_fill_manual(name = "Anaesthetic Type", values = c("lightblue", "darkcyan", "cornflowerblue", "deepskyblue2", "darkblue", "purple"))
```
...
#retro_anaes |> 
 # dplyr::filter(!anaes_type %in% c("Caudal Only", "GA + Spinal + Caudal", "GA + Caudal + Sedation", NA)) |> 
#ggplot()+
 # geom_bar(aes(x = anaes_type, fill = anaes_type), position = "dodge")+
  #theme_bw()+
  #theme(axis.text.x = element_text(angle = 90), legend.position = "none")+
  #labs(y = "Number", x = "Anaesthetic Type", title = "Anaesthetic Type")+
  #scale_fill_manual(values = c("lightblue", "darkcyan", "cornflowerblue", "deepskyblue2", "darkblue", "purple"))
...
#ggplot()+
 # geom_bar(aes(x = airway_type, fill = airway_type), position = "dodge")+
#  theme_bw()+
 # guides(fill="none")+
#  theme(axis.text.x = element_text(angle = 90), legend.position = "none")+
#  labs(y = "Number", x = "Airway Type", title = "Airway Type", subtitle = "In Patients Receiving GA")+
 # scale_x_discrete(labels = function(x) str_wrap(x, width = 10))+
#  scale_fill_manual(values = c("lightblue", "darkcyan", "cornflowerblue", "deepskyblue2", "blue", "darkblue"))
```
...
#retro_postop <- retro_postop |> 
  #mutate(
    #resp_supp_dur_mins = as.double(po_rsp_time_stop - po_rsp_time, units = "mins"),
    #resp_supp_dur_hrs = as.double(po_rsp_time_stop - po_rsp_time, units = "hours")) |> 
...
retro_postop <- retro_postop |> 
  ...
  relocate(hosp_los, .after = unplanned_icu_reason)
...
retro_postop <- retro_postop |> 
  ...
  select(record_id:surg_return_reason)



retro_postop <- retro_postop |> 
  filter(record_id != "296-2")
